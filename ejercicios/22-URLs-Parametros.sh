...
var1="/api/user?id=123&name=juan, /api/product?ref=456"
echo -e "\n${GREEN}URLS CON PARÁMETROS${NC}\n"
sleep 1
echo -e "${GREEN}Texto:${NC} $var1"
echo -e "${GREEN}Buscar:${NC}Extraer parámetros y valores"
sleep 2
...
while true; do
    ...
    code=$(echo $?)
    r1=$(echo -e "$t1" | xargs)
    r_ok=$(echo "id=123&name=juan ref=456" | xargs)
